...
def run_experiment_3(n_trials: int, db: Database, config: Config) -> Dict[str, Any]:
    """
    Experiment 3: Individual Layer Performance (RQ2)
    
    Tests each layer in isolation to measure individual contribution:
    - Config A: No Defense (baseline)
    - Config B: Layer 1 Only
    - Config C: Layer 2 Only
    - Config D: Layer 3 Only  
    - Config E: Layer 4 Only
    - Config F: Layer 5 Only
    - Config G: Full Stack (coordinated defense)
    
    Per STATISTICAL_REQUIREMENTS.md
    """
    logger.info("="*80)
    logger.info("EXPERIMENT 3: Individual Layer Performance (RQ2)")
    logger.info(f"Running with {n_trials} trial(s) per configuration")
    logger.info(f"Testing all {len(ATTACK_PROMPTS)} attack prompts + {len(BENIGN_PROMPTS)} benign prompts")
    logger.info("="*80)
    
    experiment_id = "exp3_individual_layers"
    ...
    configs = {
        "Config_A_NoDefense": {
            ...
            "enable_layer2": False,
            "enable_layer3": False,
            "enable_layer4": True,  # Layer 4 is always LLM baseline
            "enable_layer5": False,
        },
        "Config_B_Layer1Only": {
            "enable_layer1": True,
    ...
    for config_name, layer_config in configs.items():
        results[config_name] = run_config_with_trials(
            config_name=config_name,
            layer_config=layer_config,
            isolation_mode="good",  # Use good isolation for all
            attack_prompts=ATTACK_PROMPTS,
            benign_prompts=BENIGN_PROMPTS,
            n_trials=n_trials,
            experiment_id=f"{experiment_id}_{config_name}",
...
def run_experiment_4(n_trials: int, db: Database, config: Config) -> Dict[str, Any]:
    ...
    # Only test attack prompts (no benign for ablation)
    attack_prompts = ATTACK_PROMPTS
